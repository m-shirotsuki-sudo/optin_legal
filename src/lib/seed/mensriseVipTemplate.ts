/**
 * Men's Rise VIPプラン（20260519版）の本文HTMLテンプレート。
 *
 * 出所：samples/contractdemo.html を、ループ可能な `{{key}}` プレースホルダ形式へ移植したもの。
 * 本文・金額・口座・条文・赤枠注意文・赤字クーリングオフは**すべて原本ママ**で固定。
 * 入れ替わるのは `{{...}}` の中身のみ。
 *
 * 重要：このファイルを編集するときは、必ず原本docxとの照合（src/lib/checksum.ts）を通すこと。
 */
export const MENSRISE_VIP_TEMPLATE_HTML = `
<div class="notice-box">本書は、特定商取引法に基づき交付される、お客様（甲）と株式会社Men'sRise（乙）の契約条件を定める重要な書類となりますので、本契約全てのページに記載されている内容を十分にお読みください。</div>
...
/** Men's Rise VIPの可変フィールド定義（KICKOFF §6 と一致）。 */
export const MENSRISE_VIP_VARIABLE_FIELDS = [
  { key: "customer_name", label: "氏名", type: "text", required: true, placeholder: "山田 太郎" },
  { key: "customer_addr", label: "住所", type: "textarea", required: false, placeholder: "東京都〇〇区〇〇 1-2-3", hint: "任意。会社で別途記入する場合は空欄でOK" },
  { key: "customer_tel", label: "電話番号", type: "tel", required: true, hint: "数字とハイフン", placeholder: "090-1234-5678", pattern: "[0-9-]+" },
  { key: "contract_date", label: "締結年月日", type: "date", required: true },
  { key: "staff_name", label: "契約担当者氏名", type: "text", required: true, hint: "セールス担当", placeholder: "鈴木 一郎" },
  {
    group: "第4条 支払条件（該当欄のみ入力・併用可・空欄は空欄のまま出力）",
    description: "銀行振込のみ／分割のみ／両方併用、いずれもOK。使う欄だけ埋めてください。",
    fields: [
      { key: "debit_times", label: "デビットカード 支払回数（回）", type: "number", placeholder: "例：1" },
      { key: "shop_times", label: "ショッピングクレジット 支払回数（回）", type: "number", placeholder: "例：12" },
      { key: "credit_company", label: "クレジット会社名", type: "text", hint: "Mosh / CBS / Lifety 等・手入力", placeholder: "例：CBS" },
      { key: "debit_date", label: "引落 年月日", type: "text", hint: "2026年〇月〇日", placeholder: "例：6月10日" },
      { key: "bank_amount", label: "銀行振込額（円）", type: "text", placeholder: "例：660,000" },
      { key: "first_amount", label: "初回額（円・税込）", type: "text", placeholder: "例：110,000" },
      { key: "next_amount", label: "翌月額（円・税込）", type: "text", placeholder: "例：110,000" },
    ],
  },
] as const;
